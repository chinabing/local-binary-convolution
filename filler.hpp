...
/// @brief Fills a Blob with bernoulli distributed values.
template <typename Dtype>
class BernoulliFiller : public Filler<Dtype> {
 public:
  ...
  virtual void Fill(Blob<Dtype>* blob) {
    CHECK(blob->count());
    int data* = (int*)malloc(sizeof(int)*blob->count());
    Dtype *data_copy = blob->mutable_cpu_data();
    caffe_rng_bernoulli<Dtype>(blob->count(), Dtype(0.5),data);
    for(int i=0; i<blob->count(); i++){
      data_copy[i] = data[i]*2 -1;
    }
    free(data);
    CHECK_EQ(this->filler_param_.sparse(), -1)
         << "Sparsity not supported by this Filler.";
